"use client";
import Link from "next/link";
import { ChevronDown, Download, Globe, Menu, Moon, Search } from "lucide-react";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";

export default function Navbar() {
  return (
    <nav className="flex h-16 items-center font-semibold text-gray-100 justify-between bg-darkblack px-6 ">
      <div className="flex items-center gap-6">
        {/* Logo */}
        <Link href="/" className="flex items-center">
          <img
            src="data:image/png;base64,"
            alt="Binance Logo"
            className="h-[4rem]"
          />
        </Link>

        {/* Desktop Navigation */}
        <div className="hidden items-center gap-6 lg:flex">
          <Link href="/buy" className="text-sm hover:text-yellow-400">
            Buy Crypto
          </Link>
          <Link href="/markets" className="text-sm hover:text-yellow-400">
            Markets
          </Link>
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm hover:text-yellow-400">
              Trade
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem>Spot Trading</DropdownMenuItem>
              <DropdownMenuItem>Margin Trading</DropdownMenuItem>
              <DropdownMenuItem>P2P Trading</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm hover:text-yellow-400">
              Futures
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem>USDⓈ-M Futures</DropdownMenuItem>
              <DropdownMenuItem>COIN-M Futures</DropdownMenuItem>
              <DropdownMenuItem>Options</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm hover:text-yellow-400">
              Earn
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem>Binance Earn</DropdownMenuItem>
              <DropdownMenuItem>Savings</DropdownMenuItem>
              <DropdownMenuItem>Staking</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm hover:text-yellow-400">
              Square
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem>Square Features</DropdownMenuItem>
              <DropdownMenuItem>Square Trading</DropdownMenuItem>
              <DropdownMenuItem>Square Analytics</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm hover:text-yellow-400">
              More
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem>About</DropdownMenuItem>
              <DropdownMenuItem>Careers</DropdownMenuItem>
              <DropdownMenuItem>Press</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>

      {/* Right Side Navigation */}
      <div className="flex items-center gap-4">
        <Search size={20} strokeWidth={3} className="text-yellow-400" />

        <Button
          variant="ghost"
          className="hidden text-sm h-8 px-3 font-semibold bg-white/10 text-white hover:text-yellow-400 lg:inline-flex"
        >
          Log In
        </Button>
        <Button className="hidden h-8 px-3 font-semibold bg-yellow-400 text-sm text-gray-700 hover:bg-yellow-500 lg:inline-flex">
          Sign Up
        </Button>
        <Download
          fill="white"
          className="h-6 w-6 cursor-pointer hover:text-yellow-400 hover:fill-yellow-400"
        />
        <Globe
          fill="white"
          className="h-6 w-6 text-[#181a20] cursor-pointer hover:fill-yellow-400"
        />
        <Moon
          fill="white"
          className="h-5 w-5 cursor-pointer hover:text-yellow-400 hover:fill-yellow-400"
        />
        <Button variant="ghost" size="icon" className="lg:hidden">
          <Menu className="h-5 w-5" />
        </Button>
      </div>
    </nav>
  );
}
